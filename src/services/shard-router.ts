 ...
 * - ANALYTICS_DB is shard 0 (primary) - all existing data lives here
 * - SHARD_1-15 are overflow shards for future scaling
 *
 * With ACTIVE_SHARDS = 1, all orgs route to ANALYTICS_DB (shard 0).
 * Increase ACTIVE_SHARDS to distribute new orgs across additional shards.
 *
 * @example
 * ```typescript
 * const router = new ShardRouter(env.DB, getShards(env));
 * const shard = await router.getShardForOrg(orgId);
 * const campaigns = await shard.prepare('SELECT * FROM ad_campaigns WHERE organization_id = ?')
 *   .bind(orgId).all();
 * ```
 */
...
const NUM_SHARDS = 16;  // Max shards (scale up to 16)
const ACTIVE_SHARDS = 1;  // Currently active: only ANALYTICS_DB (shard 0)

export interface ShardEnv {
  ANALYTICS_DB?: D1Database;  // Shard 0 (primary)
  SHARD_1?: D1Database;       // Overflow shards (future)
  SHARD_2?: D1Database;
  SHARD_3?: D1Database;
  SHARD_4?: D1Database;
  SHARD_5?: D1Database;
 ...
 * ANALYTICS_DB is always shard 0 (primary), SHARD_1-15 are overflow
 */
export function getShards(env: ShardEnv): D1Database[] {
  const shards: D1Database[] = [];

  // ANALYTICS_DB is shard 0 (primary - all existing data)
  if (env.ANALYTICS_DB) {
    shards.push(env.ANALYTICS_DB);
  }
...
export class ShardRouter {
  ...
  /**
   * @param centralDb - Central DB for shard_routing table
   * @param shards - Array of shard databases (ANALYTICS_DB first, then SHARD_1+)
   * @param maxActiveShards - Override ACTIVE_SHARDS (for testing). Defaults to ACTIVE_SHARDS constant.
   */
  constructor(
    ...
    const effectiveMax = maxActiveShards ?? ACTIVE_SHARDS;
    this.activeShardCount = Math.min(shards.length, effectiveMax);
    if (this.activeShardCount === 0) {
      structuredLog('WARN', 'No shards available, will use fallback mode', { service: 'shard-router' });
    }
  }
