  ...
  async handle(c: AppContext) {
    ...
    try {
      ...
      const entityTableMap: Record<string, { table: string; nameColumn: string; idColumn: string }> = {
        campaign: { table: 'tiktok_campaigns', nameColumn: 'campaign_name', idColumn: 'id' },
        ad_group: { table: 'tiktok_ad_groups', nameColumn: 'ad_group_name', idColumn: 'id' },
        ad: { table: 'tiktok_ads', nameColumn: 'ad_name', idColumn: 'id' }
      };
      ...
      const metrics = (result.results || []).map((row: any) => ({
        metric_date: row.date,  // Normalize to metric_date for frontend
