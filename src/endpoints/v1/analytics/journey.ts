 ...
 *
 * NOTE: Currently returns empty journey data. Event data requires D1 tables
 * to be populated. Connector conversions (Stripe charges) are available via
 * the stripe_charges table in ANALYTICS_DB.
 */
...
export class GetJourneysOverview extends OpenAPIRoute {
  ...
  async handle(c: AppContext) {
    ...
    // TODO: Query events from D1 when events table is available
    // For now, return metrics based on identity mappings only
    console.log(`[Journey Overview] org ${orgId} - returning empty (D1 events table not yet populated)`);

    return success(c, {
      metrics: {
        total_identified_users: identityCount?.users || 0,
        total_anonymous_sessions: identityCount?.anon_ids || 0,
        identity_match_rate: 0,  // Requires events table
        avg_path_length: 0,      // Requires events table
        avg_days_to_convert: 0,  // Requires events table
        avg_sessions_to_convert: 0 // Requires events table
      },
      top_paths: [],            // Requires events table
      conversion_by_path_length: [] // Requires events table
    });
  }
}
