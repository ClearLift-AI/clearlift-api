...
-- Flow Builder tables (acquisition_instances, conversion_configs, interaction_nodes, interaction_edges, funnel_metadata)
...
CREATE TABLE conversion_configs (
  ...
  FOREIGN KEY (organization_id) REFERENCES organizations(id) ON DELETE CASCADE,
  FOREIGN KEY (goal_id) REFERENCES conversion_goals(id) ON DELETE CASCADE,
  UNIQUE(organization_id, goal_id, connector, event_type)
);

CREATE INDEX idx_conversion_configs_goal ON conversion_configs(goal_id);
CREATE INDEX idx_conversion_configs_org ON conversion_configs(organization_id);
